 ...
 ******************************************/
  class EcalClusterEnergyCorrectionParametersPlot
      : public cond::payloadInspector::PlotImage<EcalClusterEnergyCorrectionParameters> {
  public:
    EcalClusterEnergyCorrectionParametersPlot()
        : cond::payloadInspector::PlotImage<EcalClusterEnergyCorrectionParameters>(
              "Ecal Cluster Energy Correction Parameters - map ") {
      setSingleIov(true);
    }

    bool fill(const std::vector<std::tuple<cond::Time_t, cond::Hash> >& iovs) override {
      auto iov = iovs.front();  //get reference to 1st element in the vector iovs
      std::shared_ptr<EcalClusterEnergyCorrectionParameters> payload =
      ...
      if (payload.get()) {  //payload is an iov retrieved from payload using hash.
        ...
        fillFunctionParamsValues(
            align, (*payload).params(), "Ecal Cluster Energy Correction Parameters", gridRows, NbColumns);
