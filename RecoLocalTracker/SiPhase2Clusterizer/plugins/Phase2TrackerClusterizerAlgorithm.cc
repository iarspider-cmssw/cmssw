...
Phase2TrackerClusterizerAlgorithm::Phase2TrackerClusterizerAlgorithm(unsigned int maxClusterSize,
    ...
    : maxClusterSize_(maxClusterSize), maxNumberClusters_(maxNumberClusters), nrows_(0), ncols_(0) {}
...
void Phase2TrackerClusterizerAlgorithm::clusterizeDetUnit(const edm::DetSet<Phase2TrackerDigi>& digis,
  ...
  for (unsigned int col(0); col < ncols_; ++col) {
    for (unsigned int row(0); row < nrows_; ++row) {
      ...
      if (maxNumberClusters_ != 0 and numberClusters > maxNumberClusters_)
        return;
    }
  }
