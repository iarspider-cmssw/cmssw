...
void CSCAlignmentCorrections::plot() {
  ...
  for (unsigned int i = 0; i < m_coefficient.size(); i++) {
    ...
    TH1F *hist = tFileService->make<TH1F>(
        histname.str().c_str(), histtitle.str().c_str(), m_coefficient[i].size(), 0.5, m_coefficient[i].size() + 0.5);
...
void CSCAlignmentCorrections::report(std::ofstream &report) {
  report << "cscReports.append(CSCFitterReport(\"" << m_fitterName << "\", " << m_oldchi2 << ", " << m_newchi2 << "))"
         << std::endl;

  for (unsigned int i = 0; i < m_name.size(); i++) {
    report << "cscReports[-1].addChamberCorrection(\"" << m_name[i] << "\", " << m_id[i].rawId() << ", " << m_value[i]
           << ")" << std::endl;
  }
  ...
  for (unsigned int i = 0; i < m_coefficient.size(); i++) {
    ...
    for (unsigned int j = 0; j < m_coefficient[i].size(); j++) {
      report << "cscReports[-1].addErrorModeTerm(\"" << m_modename[i][j] << "\", " << m_modeid[i][j] << ", "
             << m_coefficient[i][j] << ")" << std::endl;
    }
  }

  for (unsigned int i = 0; i < m_i.size(); i++) {
    report << "cscReports[-1].addCSCConstraintResidual(\"" << m_i[i] << "\", \"" << m_j[i] << "\", " << m_before[i]
           << ", " << m_uncert[i] << ", " << m_residual[i] << ", " << m_pull[i] << ")" << std::endl;
  }
...
void CSCAlignmentCorrections::applyAlignment(AlignableNavigator *alignableNavigator,
  ...
  for (unsigned int i = 0; i < m_name.size(); i++) {
    ...
    bool backward =
        ((m_id[i].endcap() == 1 && m_id[i].station() >= 3) || (m_id[i].endcap() == 2 && m_id[i].station() < 3));
