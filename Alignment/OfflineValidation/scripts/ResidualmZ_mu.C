...
//&&&&&&&&&&&&&&&&&&&&&&&&&&&&&
hframe = new TH2F("hframe","#Delta (m_{Z})",100,-10.,10.,32,0.,0.125);
hframe->SetXTitle("#Delta(m_{Z}) [GeV/c^{2}]");
hframe->SetYTitle("N. ev. / 0.2");
hframe->Draw();
...
for(int i=0; i<6; i++){
  ...
  mZmu[i] = new TH1F(name,name,100,-10.,10.);
  sprintf(histoname,"mZmu[%d]",i);

  MyTree[i]->Project(histoname,"recmzmu-mzmu","eff==1 && mzmu>0. && recmzmu>0.");
  
  cout << "Entries " << mZmu[i]->GetEntries() <<endl;
  mZmu[i]->Scale(1/mZmu[i]->GetEntries());
  mZmu[i]->SetTitle("Z->#mu#mu mass residual from Z->#mu#mu");    
...
c1->SaveAs("Residual_mZ_mu.gif");
c1->SaveAs("Residual_mZ_mu.eps");
gROOT->Reset();
gROOT->Clear();
