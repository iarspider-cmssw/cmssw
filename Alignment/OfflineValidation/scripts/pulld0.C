...
for(int i=0; i<4; i++){
  sprintf(name,"pulld0[%d]",i);
  pulld0[i] = new TH1F(name,name,140,-10,10);
  sprintf(histoname,"pulld0[%d]",i);
  MyTree[i]->Project(histoname,"pulld0","eff==1 && TrackID==13");

  cout << "Entries " << pulld0[i]->GetEntries() <<endl;
  pulld0[i]->Scale(1/pulld0[i]->GetEntries());
  pulld0[i]->SetTitle("pull(d_{0})");    
  pulld0[i]->SetXTitle("pull(d_{0}) ");
  pulld0[i]->SetYTitle("arb. units");
  
  pulld0[i]->SetLineColor(i+2);
  pulld0[i]->SetLineStyle(i+1);
  pulld0[i]->SetLineWidth(i+2);
  if (i==0) pulld0[i]->Draw();
  else pulld0[i]->Draw("same");
  //  c1->WaitPrimitive();
  c1->Update();
}

Legend("pulld0[0]","pulld0[1]","pulld0[2]","pulld0[3]",pulld0[0],pulld0[1],pulld0[2],pulld0[3]);

c1->SaveAs("pulld0_mu.eps");
c1->SaveAs("pulld0_mu.gif");
gROOT->Reset();
gROOT->Clear();
