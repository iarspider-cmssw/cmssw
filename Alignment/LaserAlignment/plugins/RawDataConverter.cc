...
///
///
void RawDataConverter::beginJob() { fillDetectorId(); }
...
RawDataConverter::DigiType RawDataConverter::GetValidLabels(
    const edm::Event& iEvent)  // Check what kind of file is being processed and get valid module and instance labels
{
...
void RawDataConverter::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup) {
  ...
  if (digitype == Unknown)
    digitype = GetValidLabels(iEvent);  // Initialization of Digi Type
