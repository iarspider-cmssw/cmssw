...
MatrixMeschach& MatrixMeschach::operator=(const MatrixMeschach& mat) {
  ...
  if (ALIUtils::debug >= 9)
    std::cout << "operator=" << mat._Mat << _Mat << NoLines() << NoColumns() << Mat()->m << Mat()->n << std::endl;
  return *this;
}
...
void MatrixMeschach::operator*=(const MatrixMeschach& mat) {
  ...
  if (_NoColumns != mat.NoLines()) {
    std::cerr << " !! Trying two multiply two matrices when the number of columns of first one is not equal to number "
                 "of files of second one "
              << std::endl;
    std::cout << " multiplying matrix " << _NoLines << " x " << _NoColumns << " and " << mat.NoLines() << " x "
              << mat.NoColumns() << " results in " << _NoLines << " x " << mat.NoColumns() << std::endl;
  }
...
MatrixMeschach operator*(const MatrixMeschach& mat1, const MatrixMeschach& mat2) {
  ...
  if (mat1copy.NoColumns() != mat2.NoLines()) {
    std::cerr << " !! Trying two multiply two matrices when the number of columns of first one is not equal to number "
                 "of files of second one "
              << std::endl;
    std::cout << " multiplying matrix " << mat1copy.NoLines() << " x " << mat1copy.NoColumns() << " and "
              << mat2.NoLines() << " x " << mat2.NoColumns() << " results in " << mat1copy.NoLines() << " x "
              << mat2.NoColumns() << std::endl;
  }
...
MatrixMeschach operator+(const MatrixMeschach& mat1, const MatrixMeschach& mat2) {
  ...
  matout += mat2;
  if (ALIUtils::debug >= 9)
    std::cout << "add" << mat1.NoLines() << mat1.NoColumns() << mat2.NoLines() << mat2.NoColumns() << matout.NoLines()
              << matout.NoColumns() << std::endl;
  if (ALIUtils::debug >= 9)
    std::cout << "addM" << mat1.Mat()->m << mat1.Mat()->n << mat2.Mat()->m << mat2.Mat()->n << matout.Mat()->m
              << matout.Mat()->n << std::endl;
  return MatrixMeschach(matout);
}
...
//@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
MatrixMeschach operator*(const MatrixMeschach& mat, const ALIdouble doub) { return doub * mat; }
...
void MatrixMeschach::inverse() {
  if (ALIUtils::debug >= 9)
    std::cout << "inverse" << _NoLines << "C" << _NoColumns << std::endl;
...
void MatrixMeschach::AddData(ALIuint lin, ALIuint col, ALIdouble data) {
  if (lin >= _Mat->m || col >= _Mat->n) {
    std::cerr << "EXITING: matrix has only " << _NoLines << " lines and " << _NoColumns << " columns " << std::endl;
    std::cerr << "EXITING: matrix has only " << _Mat->m << " lines and " << _Mat->n << " columns " << std::endl;
    std::cerr << " You tried to add data in line " << lin << " and column " << col << std::endl;
    std::exception();
  }
...
//@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
ALIdouble MatrixMeschach::operator()(int i, int j) const { return _Mat->me[i][j]; }
...
void MatrixMeschach::EliminateLines(ALIint lin_first, ALIint lin_last) {
  ...
  if (lin_last < lin_first) {
    std::cerr << "EXITING: cannot Eliminate Lines in matrix if first line is " << lin_first << " and lastt line is "
              << lin_last << std::endl;
    //t          std::exception();
    return;
  }
  ...
  for (ALIint ii = 0; ii < NoLines(); ii++) {
    if (ii < lin_first || ii > lin_last) {
      for (ALIint jj = 0; jj < NoColumns(); jj++) {
        ...
        if (ALIUtils::debug >= 9)
          std::cout << iin << jj << "nmat" << newmat.Mat()->me[iin][jj] << std::endl;
      }
      iin++;
    }
  }
  ...
  for (ALIint jj = 0; jj < NoColumns(); jj++) {
    if (jj < lin_first || jj > lin_last) {
      for (ALIint ii = 0; ii < NoLines(); ii++) {
        ...
        if (ALIUtils::debug >= 9)
          std::cout << ii << jjn << "nmat" << newmat.Mat()->me[ii][jjn] << std::endl;
      }
      jjn++;
    }
  }
...
//@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
void MatrixMeschach::Dump(const ALIstring& mtext) { ostrDump(std::cout, mtext); }
...
void MatrixMeschach::SetCorrelation(ALIint i1, ALIint i2, ALIdouble corr) {
  ...
  if (ALIUtils::debug >= 9)
    std::cout << i1 << i2 << corr << "CORR" << (*this)(i1, i1) << " " << (*this)(i2, i2) << std::endl;
  //-  std::cout << (*this)(i1,i1)*(*this)(i2,i2)  << std::endl;
  //- std::cout << sqrt( (*this)(i1,i1)*(*this)(i2,i2) ) << std::endl;
  if (ALIUtils::debug >= 9)
    std::cout << corr * sqrt((*this)(i1, i1) * (*this)(i2, i2)) << std::endl;
}
