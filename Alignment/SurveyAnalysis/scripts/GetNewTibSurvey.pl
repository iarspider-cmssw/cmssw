...
for ( my $iLayer = 1; $iLayer <= $nTibLayers; $iLayer++ ) {
  foreach my $zSide ( @zSides ) {
    foreach my $strSide ( @strSides )  {
      ...
      my @id = ( $iLayer, ($zSide=~/-/?1:2), ($strSide=~/int/?1:2) ); # See TIBDetId
  ...
  for ( my $iDisk = 1; $iDisk<=$nTidDisks; $iDisk++ ) {
    foreach my $zSide ( @zSides ) {
      foreach my $dSide ( @dSides ) {
        ...
        my @geomData = &extractData( $geometry );
        my @surveyData = &extractData( $survey );
        my @id = ( ($zSide=~/-/?1:2), $iDisk, $iRing, ($dSide=~/back/?1:2) ); # See TIDDetId
  ...
  foreach my $gLine ( @$geometry )
    {
      ...
      printf OUTPUT "%9d", shift @gFields;  # DetId first (check only)
      print OUTPUT " ";

      foreach my $field ( @$id ) {
        printf OUTPUT "%2d ", $field;   # Subdet specific stuff
      }
      ...
      shift @sFields; shift @sFields; shift @sFields; 
