...
int MillePedeMonitor::GetIndex(const std::vector<OBJECT_TYPE *> &vec, const TString &name) {
  ...
  edm::LogError("Alignment") << "@SUB=MillePedeMonitor::GetIndex"
                             << " could not find " << name;
  return -1;
}
...
std::vector<OBJECT_TYPE *> MillePedeMonitor::cloneHists(const std::vector<OBJECT_TYPE *> &orgs,
  ...
  for (typename std::vector<OBJECT_TYPE *>::const_iterator iter = orgs.begin(), iterEnd = orgs.end(); iter != iterEnd;
    ...
    if (result.back())
      result.back()->SetTitle((*iter)->GetTitle() + titAd);
    else
      edm::LogError("Alignment") << "@SUB=MillePedeMonitor::cloneHists"
                                 << "out of memory?";
  }
...
template <typename T, size_t SIZE>
std::array<int, SIZE> MillePedeMonitor::indexArray1D(const std::vector<T *> &hists, const char *title) {
