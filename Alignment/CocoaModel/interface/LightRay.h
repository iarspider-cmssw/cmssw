...
public:
  ...
  void shiftAndDeviateWhileTraversing(const OpticalObject* opto,
                                      ALIdouble shiftX,
                                      ALIdouble shiftY,
                                      ALIdouble shiftZ,
                                      ALIdouble deviX,
  ...
  const CLHEP::Hep3Vector& direction() const { return _direction; }
  void dumpData(const ALIstring& str) const;

  // SET DATA MEMBERS
  void setDirection(const CLHEP::Hep3Vector& direc) { _direction = direc; }
  void setPoint(const CLHEP::Hep3Vector& point) { _point = point; }
