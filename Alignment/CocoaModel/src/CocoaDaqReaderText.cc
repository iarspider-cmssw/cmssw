...
CocoaDaqReaderText::CocoaDaqReaderText(const std::string& fileName) {
  if (ALIUtils::debug >= 5)
    std::cout << " CocoaDaqReaderText::CocoaDaqReaderText from file " << fileName << std::endl;
...
bool CocoaDaqReaderText::ReadNextEvent() {
  ...
  if (ALIUtils::debug >= 4) {
    std::cout << " Reading " << nMeas << " measurements from file " << theFilein.name() << " DATE: " << wordlist[1]
              << " " << wordlist[1] << std::endl;
  }
