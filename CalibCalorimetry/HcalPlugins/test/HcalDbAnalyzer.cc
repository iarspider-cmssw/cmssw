  ...
  std::ostream& operator<<(std::ostream& fOut, const DetId& id) {
    if (id.null())
      ...
      if (sub == HcalBarrel || sub == HcalEndcap || sub == HcalOuter || sub == HcalForward)
        fOut << HcalDetId(id);
      else if (sub == HcalEmpty)
        fOut << "EMPTY";
      else if (sub == HcalTriggerTower)
        fOut << HcalTrigTowerDetId(id);
      else if (sub == HcalOther) {
        HcalOtherDetId osub(id);
        HcalOtherSubdetector odetid = osub.subdet();
        if (odetid == HcalCalibration)
          fOut << HcalCalibDetId(id);
        else
          fOut << "Unknown subtype";
      } else
        fOut << "Unknown type";
    }
    return fOut;
...
void HcalDbAnalyzer::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup) {
  ...
  std::cout << "Values-> pedestals: " << calibrations.pedestal(0) << '/' << calibrations.pedestal(1) << '/'
            << calibrations.pedestal(2) << '/' << calibrations.pedestal(3) << ",  gains: " << calibrations.rawgain(0)
            << '/' << calibrations.rawgain(1) << '/' << calibrations.rawgain(2) << '/' << calibrations.rawgain(3)
            << std::endl;
  std::cout << "Widths. pedestals: " << widths.pedestal(0) << '/' << widths.pedestal(1) << '/' << widths.pedestal(2)
            << '/' << widths.pedestal(3) << ",  gains: " << widths.gain(0) << '/' << widths.gain(1) << '/'
            << widths.gain(2) << '/' << widths.gain(3) << std::endl;
  ...
  for (int i = 0; i < 128; i++) {
    ...
    std::cout << " reverse ADC: " << i << " q1:" << adc0 << " q2:" << adc1 << " q3:" << adc2 << " q4:" << adc3
              << std::endl;
  }
