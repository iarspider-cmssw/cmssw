...
DOMDocument* XMLDOMBlock::getDocument(void) { return document; }
...
const char* XMLDOMBlock::getTagValue(const std::string& tagName, int _item, DOMDocument* _document) {
  if (!_document)
    _document = document;
  const char* _result = XMLString::transcode(
      _document->getElementsByTagName(XMLProcessor::_toXMLCh(tagName))->item(_item)->getFirstChild()->getNodeValue());
  return _result;
}
...
DOMNode* XMLDOMBlock::setTagAttribute(
    DOMElement* _elem, const std::string& tagName, const std::string& attrName, const int& attrValue, int _item) {
  if (!_elem)
    return nullptr;
  DOMNode* the_tag = _elem->getElementsByTagName(XMLProcessor::_toXMLCh(tagName))->item(_item);
  if (the_tag) {
...
std::string& XMLDOMBlock::getString(void) { return getString(this->getDocument()); }
