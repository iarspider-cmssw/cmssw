...
bool HcalDbXml::dumpObject(std::ostream& fOutput,
  ...
  for (std::vector<DetId>::iterator channel = channels.begin(); channel != channels.end(); ++channel) {
    ...
    if (!values) {
      std::cerr << "HcalDbXml::dumpObject-> Can not get data for channel " << HcalText2DetIdConverter(chId).toString()
                << std::endl;
      continue;
    }
    if (!errors) {
      std::cerr << "HcalDbXml::dumpObject-> Can not get errors for channel " << HcalText2DetIdConverter(chId).toString()
