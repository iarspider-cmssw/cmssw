 ...
 */
  ...
  for (unsigned int col = 0; col < tableMat[1].size(); col++)  //Goes to every column of the Matrix
  {
    ...
    if (type_ == "fpix" && settingName.find("AOH_") != string::npos &&
        settingName.find("GAIN") != string::npos  // contains both "AOH_" and "Gain"
        && settingName.find("123") == string::npos &&
        settingName.find("456") == string::npos)  // does not contain "123" or "456"
    {
    ...
    } else if (type_ == "bpix" && settingName.find("AOH") != string::npos &&
               settingName.find("GAIN") != string::npos     // contains both "AOH" and "Gain"
               ...
               settingName.find("456") == string::npos)  // does not contain "123" or "456"
    {
      if (portcardname_.find("PRT2") != std::string::npos &&
          (settingName.find("AOH3_") != std::string::npos || settingName.find("AOH4_") != std::string::npos))
        continue;
      setDataBaseAOHGain(settingName, i2c_values);
      //	  cout << __LINE__ << "]\t" << mthn << "Setting " << settingName << "\tto value " << std::hex << i2c_values << std::dec << std::endl ;
    }
    ...
    else if (settingName == k_PLL_CTR5)  // special handling
    {
      ...
      if (containsSetting(k_PLL_CTR2))
        last_CTR2 = getdeviceValuesForSetting(k_PLL_CTR2);
    ...
    {
      if ((settingName.find("DELAY25_") != std::string::npos) || (settingName.find("_BIAS") != std::string::npos) ||
          (settingName.find("PLL_CTR2") != std::string::npos) || (settingName.find("PLL_CTR5") != std::string::npos) ||
      ...
      //Note that DOH_SEU_GAIN will be *ignored* for fpix
      {
        ...
        if (type_ == "fpix" &&
            (settingName.find("AOH1_") != std::string::npos || settingName.find("AOH2_") != std::string::npos ||
             settingName.find("AOH3_") != std::string::npos || settingName.find("AOH4_") != std::string::npos))
          continue;
        ...
        /*
	      ...
	      */
        device_.push_back(p);
        if (settingName.find("AOH") != string::npos)
          key_.push_back(aohcount_++);
        else if (settingName.find("Delay25") != string::npos)
          key_.push_back(delay25count++);
        else if (settingName.find("PLL") != string::npos)
          key_.push_back(pllcount++);
        else
          key_.push_back(othercount++);
      }
    }
...
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
PixelPortCardConfig::PixelPortCardConfig(std::string filename) : PixelConfigBase(" ", " ", " ") {
  string mthn = "[PixelPortCardConfig::PixelPortCardConfig()]\t\t    ";
  ...
  fillNameToAddress();
  fillDBToFileAddress();
  assert(dummy == "TKFECID:");
  in >> TKFECID_;
  in >> dummy;
  assert(dummy == "ringAddress:");
  in >> std::hex >> ringAddress_;
  in >> dummy;
  assert(dummy == "ccuAddress:");
  in >> std::hex >> ccuAddress_;
  in >> dummy;
  assert(dummy == "channelAddress:");
  in >> std::hex >> channelAddress_;
  in >> dummy;
  assert(dummy == "i2cSpeed:");
  in >> std::hex >> i2cSpeed_;
  ...
  do {
    ...
    if (settingName[settingName.size() - 1] == ':')
      settingName.resize(settingName.size() - 1);  // remove ':' from end of string, if it's there
...
unsigned int PixelPortCardConfig::new_PLL_CTR2_value(std::string CTR4or5, unsigned int last_CTR2) const {
  if (CTR4or5 == k_PLL_CTR4)
    return 0xdf & last_CTR2;
  else if (CTR4or5 == k_PLL_CTR5)
...
void PixelPortCardConfig::setAOHGain(std::string settingName, unsigned int value) {
  assert(settingName.find("AOH") != string::npos &&
         settingName.find("Gain") != string::npos  // contains both "AOH" and "Gain"
         && settingName.find("123") == string::npos &&
         settingName.find("456") == string::npos);  // does not contain "123" or "456"
  ...
  if (settingName[GainPosition - 2] == 'H')
    whichAOH = 0;  // fpix
  else             // bpix
  {
  ...
  if (whichAOH == 0 && channelOnAOH <= 3)
    i2c_address = k_fpix_AOH_Gain123_address;
  else if (whichAOH == 0 && channelOnAOH >= 4)
    i2c_address = k_fpix_AOH_Gain456_address;
  else if (whichAOH == 1 && channelOnAOH <= 3)
    i2c_address = k_bpix_AOH1_Gain123_address;
  else if (whichAOH == 1 && channelOnAOH >= 4)
    i2c_address = k_bpix_AOH1_Gain456_address;
  else if (whichAOH == 2 && channelOnAOH <= 3)
    i2c_address = k_bpix_AOH2_Gain123_address;
  else if (whichAOH == 2 && channelOnAOH >= 4)
    i2c_address = k_bpix_AOH2_Gain456_address;
  else if (whichAOH == 3 && channelOnAOH <= 3)
    i2c_address = k_bpix_AOH3_Gain123_address;
  else if (whichAOH == 3 && channelOnAOH >= 4)
    i2c_address = k_bpix_AOH3_Gain456_address;
  else if (whichAOH == 4 && channelOnAOH <= 3)
    i2c_address = k_bpix_AOH4_Gain123_address;
  else if (whichAOH == 4 && channelOnAOH >= 4)
    i2c_address = k_bpix_AOH4_Gain456_address;
  else
    assert(0);
  ...
  else  // If this was not set previously, add this setting with the other two gains set to zero.
  {
    ...
    if (channelOnAOH % 3 == 1)
      i2c_value = ((value & 0x3) << 0);
    else if (channelOnAOH % 3 == 2)
      i2c_value = ((value & 0x3) << 2);
    else if (channelOnAOH % 3 == 0)
      i2c_value = ((value & 0x3) << 4);
    else
      assert(0);
  ...
  for (unsigned int i = 0; i < device_.size(); i++) {
    ...
    if ((type_ == "fpix" && deviceAddress == k_fpix_AOH_Gain123_address) ||
      ...
      if (type_ == "fpix" && deviceAddress == k_fpix_AOH_Gain123_address) {
        whichAOHString = "";
        zeroOrThree = 0;
      } else if (type_ == "fpix" && deviceAddress == k_fpix_AOH_Gain456_address) {
        whichAOHString = "";
        zeroOrThree = 3;
      } else if (type_ == "bpix" && deviceAddress == k_bpix_AOH1_Gain123_address) {
        whichAOHString = "1";
        zeroOrThree = 0;
      } else if (type_ == "bpix" && deviceAddress == k_bpix_AOH1_Gain456_address) {
        whichAOHString = "1";
        zeroOrThree = 3;
      } else if (type_ == "bpix" && deviceAddress == k_bpix_AOH2_Gain123_address) {
        whichAOHString = "2";
        zeroOrThree = 0;
      } else if (type_ == "bpix" && deviceAddress == k_bpix_AOH2_Gain456_address) {
        whichAOHString = "2";
        zeroOrThree = 3;
      } else if (type_ == "bpix" && deviceAddress == k_bpix_AOH3_Gain123_address) {
        whichAOHString = "3";
        zeroOrThree = 0;
      } else if (type_ == "bpix" && deviceAddress == k_bpix_AOH3_Gain456_address) {
        whichAOHString = "3";
        zeroOrThree = 3;
      } else if (type_ == "bpix" && deviceAddress == k_bpix_AOH4_Gain123_address) {
        whichAOHString = "4";
        zeroOrThree = 0;
      } else if (type_ == "bpix" && deviceAddress == k_bpix_AOH4_Gain456_address) {
        whichAOHString = "4";
        zeroOrThree = 3;
      } else
        assert(0);
    ...
    {
      if ((last_PLL_CTR2_value & 0x20) == 0x0)
        settingName = k_PLL_CTR4;
      else
        settingName = k_PLL_CTR5;
    }
    // end of special handling
...
unsigned int PixelPortCardConfig::getdevicesize() const { return device_.size(); }

std::string PixelPortCardConfig::getTKFECID() const { return TKFECID_; }

unsigned int PixelPortCardConfig::getringAddress() const { return ringAddress_; }

unsigned int PixelPortCardConfig::getccuAddress() const { return ccuAddress_; }

unsigned int PixelPortCardConfig::getchannelAddress() const { return channelAddress_; }

unsigned int PixelPortCardConfig::geti2cSpeed() const { return i2cSpeed_; }

std::string PixelPortCardConfig::gettype() const { return type_; }
...
void PixelPortCardConfig::setdeviceValues(unsigned int address, unsigned int value) {
  for (int i = device_.size() - 1; i >= 0;
       i--)  // Change only the last occurance of address, if there are more than one.
  {
...
unsigned int PixelPortCardConfig::AOHBiasAddressFromAOHNumber(unsigned int AOHNumber) const {
  ...
  if (type_ == "fpix") {
    if (AOHNumber == 1)
      return PortCardSettingNames::k_fpix_AOH_Bias1_address;
    else if (AOHNumber == 2)
      return PortCardSettingNames::k_fpix_AOH_Bias2_address;
    else if (AOHNumber == 3)
      return PortCardSettingNames::k_fpix_AOH_Bias3_address;
    else if (AOHNumber == 4)
      return PortCardSettingNames::k_fpix_AOH_Bias4_address;
    else if (AOHNumber == 5)
      return PortCardSettingNames::k_fpix_AOH_Bias5_address;
    else if (AOHNumber == 6)
      return PortCardSettingNames::k_fpix_AOH_Bias6_address;
    else {
      std::cout << __LINE__ << "]\t" << mthn
    ...
    if (AOHNumber == 1)
      return PortCardSettingNames::k_bpix_AOH1_Bias1_address;
    else if (AOHNumber == 2)
      return PortCardSettingNames::k_bpix_AOH1_Bias2_address;
    else if (AOHNumber == 3)
      return PortCardSettingNames::k_bpix_AOH1_Bias3_address;
    else if (AOHNumber == 4)
      return PortCardSettingNames::k_bpix_AOH1_Bias4_address;
    else if (AOHNumber == 5)
      return PortCardSettingNames::k_bpix_AOH1_Bias5_address;
    else if (AOHNumber == 6)
      return PortCardSettingNames::k_bpix_AOH1_Bias6_address;
    else if (AOHNumber == 7)
      return PortCardSettingNames::k_bpix_AOH2_Bias1_address;
    else if (AOHNumber == 8)
      return PortCardSettingNames::k_bpix_AOH2_Bias2_address;
    else if (AOHNumber == 9)
      return PortCardSettingNames::k_bpix_AOH2_Bias3_address;
    else if (AOHNumber == 10)
      return PortCardSettingNames::k_bpix_AOH2_Bias4_address;
    else if (AOHNumber == 11)
      return PortCardSettingNames::k_bpix_AOH2_Bias5_address;
    else if (AOHNumber == 12)
      return PortCardSettingNames::k_bpix_AOH2_Bias6_address;
    else if (AOHNumber == 13)
      return PortCardSettingNames::k_bpix_AOH3_Bias1_address;
    else if (AOHNumber == 14)
      return PortCardSettingNames::k_bpix_AOH3_Bias2_address;
    else if (AOHNumber == 15)
      return PortCardSettingNames::k_bpix_AOH3_Bias3_address;
    else if (AOHNumber == 16)
      return PortCardSettingNames::k_bpix_AOH3_Bias4_address;
    else if (AOHNumber == 17)
      return PortCardSettingNames::k_bpix_AOH3_Bias5_address;
    else if (AOHNumber == 18)
      return PortCardSettingNames::k_bpix_AOH3_Bias6_address;
    else if (AOHNumber == 19)
      return PortCardSettingNames::k_bpix_AOH4_Bias1_address;
    else if (AOHNumber == 20)
      return PortCardSettingNames::k_bpix_AOH4_Bias2_address;
    else if (AOHNumber == 21)
      return PortCardSettingNames::k_bpix_AOH4_Bias3_address;
    else if (AOHNumber == 22)
      return PortCardSettingNames::k_bpix_AOH4_Bias4_address;
    else if (AOHNumber == 23)
      return PortCardSettingNames::k_bpix_AOH4_Bias5_address;
    else if (AOHNumber == 24)
      return PortCardSettingNames::k_bpix_AOH4_Bias6_address;
    else {
      std::cout << __LINE__ << "]\t" << mthn
...
unsigned int PixelPortCardConfig::AOHGainAddressFromAOHNumber(unsigned int AOHNumber) const {
  ...
  if (type_ == "fpix") {
    if (AOHNumber == 1)
      address = PortCardSettingNames::k_fpix_AOH_Gain123_address;
    else if (AOHNumber == 2)
      address = PortCardSettingNames::k_fpix_AOH_Gain123_address;
    else if (AOHNumber == 3)
      address = PortCardSettingNames::k_fpix_AOH_Gain123_address;
    else if (AOHNumber == 4)
      address = PortCardSettingNames::k_fpix_AOH_Gain456_address;
    else if (AOHNumber == 5)
      address = PortCardSettingNames::k_fpix_AOH_Gain456_address;
    else if (AOHNumber == 6)
      address = PortCardSettingNames::k_fpix_AOH_Gain456_address;
    ...
    if (AOHNumber == 1)
      address = PortCardSettingNames::k_bpix_AOH1_Gain123_address;
    else if (AOHNumber == 2)
      address = PortCardSettingNames::k_bpix_AOH1_Gain123_address;
    else if (AOHNumber == 3)
      address = PortCardSettingNames::k_bpix_AOH1_Gain123_address;
    else if (AOHNumber == 4)
      address = PortCardSettingNames::k_bpix_AOH1_Gain456_address;
    else if (AOHNumber == 5)
      address = PortCardSettingNames::k_bpix_AOH1_Gain456_address;
    else if (AOHNumber == 6)
      address = PortCardSettingNames::k_bpix_AOH1_Gain456_address;
    else if (AOHNumber == 7)
      address = PortCardSettingNames::k_bpix_AOH2_Gain123_address;
    else if (AOHNumber == 8)
      address = PortCardSettingNames::k_bpix_AOH2_Gain123_address;
    else if (AOHNumber == 9)
      address = PortCardSettingNames::k_bpix_AOH2_Gain123_address;
    else if (AOHNumber == 10)
      address = PortCardSettingNames::k_bpix_AOH2_Gain456_address;
    else if (AOHNumber == 11)
      address = PortCardSettingNames::k_bpix_AOH2_Gain456_address;
    else if (AOHNumber == 12)
      address = PortCardSettingNames::k_bpix_AOH2_Gain456_address;
    else if (AOHNumber == 13)
      address = PortCardSettingNames::k_bpix_AOH3_Gain123_address;
    else if (AOHNumber == 14)
      address = PortCardSettingNames::k_bpix_AOH3_Gain123_address;
    else if (AOHNumber == 15)
      address = PortCardSettingNames::k_bpix_AOH3_Gain123_address;
    else if (AOHNumber == 16)
      address = PortCardSettingNames::k_bpix_AOH3_Gain456_address;
    else if (AOHNumber == 17)
      address = PortCardSettingNames::k_bpix_AOH3_Gain456_address;
    else if (AOHNumber == 18)
      address = PortCardSettingNames::k_bpix_AOH3_Gain456_address;
    else if (AOHNumber == 19)
      address = PortCardSettingNames::k_bpix_AOH4_Gain123_address;
    else if (AOHNumber == 20)
      address = PortCardSettingNames::k_bpix_AOH4_Gain123_address;
    else if (AOHNumber == 21)
      address = PortCardSettingNames::k_bpix_AOH4_Gain123_address;
    else if (AOHNumber == 22)
      address = PortCardSettingNames::k_bpix_AOH4_Gain456_address;
    else if (AOHNumber == 23)
      address = PortCardSettingNames::k_bpix_AOH4_Gain456_address;
    else if (AOHNumber == 24)
      address = PortCardSettingNames::k_bpix_AOH4_Gain456_address;
