...
*/
  ...
  for (unsigned int r = 1; r < tableMat.size(); r++) {  //Goes to every row of the Matrix
    ...
    fedname = tableMat
        [r]
        [colM["PIXEL_FED"]];  //This is not going to work if you change in the database "PxlFed_#" in the FED column.Im removing "PlxFed_" and store the number
    ...
    if (fedconfig_.empty()) {
      ...
      if (flag == false) {
        PixelFEDParameters tmp;
        tmp.setFEDParameters(fednum,
                             (unsigned int)atoi(tableMat[r][colM["CRATE_NUMBER"]].c_str()),
...
PixelFEDConfig::PixelFEDConfig(std::string filename) : PixelConfigBase(" ", " ", " ") {
  std::string mthn = "[PixelFEDConfig::PixelFEDConfig()]\t\t\t    ";
...
unsigned int PixelFEDConfig::getNFEDBoards() const { return fedconfig_.size(); }
