...
*/
  ...
  */
   ...
   */
    ...
    if (ROCNameFromFEDChannelROCExists(fednumber, fedchannel, fedrocnumber)) {
      std::cout << __LINE__ << "]\t" << mthn << "ROC with fednumber=" << fednumber << " fedchannel=" << fedchannel
                << " roc number=" << fedrocnumber << " already exists" << std::endl;
      std::cout << __LINE__ << "]\t" << mthn << "Fix this inconsistency in the name translation" << std::endl;
      assert(0);
    }

    PixelHdwAddress hdwAdd(
        fecnumber, mfec, mfecchannel, hubaddress, portaddress, rocid, fednumber, fedchannel, fedrocnumber);
    ...
    if (channelTranslationTable_itr != channelTranslationTable_.end()) {
      ...
      assert(channelTranslationTable_itr->second.fednumber() != hdwAdd.fednumber() ||
             channelTranslationTable_itr->second.fedchannel() != hdwAdd.fedchannel());
    }
  ...
  for (; it != translationtable_.end(); ++it) {
    ...
    if (index > maxindex[fednumber][fedchannel])
      maxindex[fednumber][fedchannel] = index;
    tmp[index] = it->first;
...
PixelNameTranslation::PixelNameTranslation(std::string filename) : PixelConfigBase(" ", " ", " ") {
  ...
  do {
    ...
    if (TBMChannel != "A" &&
        TBMChannel !=
            "B")  // no TBM channel was specified, so default to A and set fecnumber to the value of this string
    {
    ...
    in >> mfec >> mfecchannel >> hubaddress >> portaddress >> rocid >> fednumber >> fedchannel >> fedrocnumber;
...
std::list<const PixelModuleName*> PixelNameTranslation::getModules() const {
  ...
  for (std::map<PixelChannel, PixelHdwAddress>::const_iterator channelTranslationTable_itr =
    ...
    if (!foundOne)
      listOfModules.push_back(&(channelTranslationTable_itr->first.module()));
  }
...
std::set<PixelChannel> PixelNameTranslation::getChannelsOnModule(const PixelModuleName& aModule) const {
  ...
  for (std::map<PixelChannel, PixelHdwAddress>::const_iterator channelTranslationTable_itr =
    ...
    if (channelTranslationTable_itr->first.module() == aModule)
      returnThis.insert(channelTranslationTable_itr->first);
  }
...
PixelChannel PixelNameTranslation::ChannelFromFEDChannel(unsigned int fednumber, unsigned int fedchannel) const {
  ...
  if (!foundOne) {
    std::cout << __LINE__ << "]\t" << mthn << "ERROR: no channel found for FED#" << fednumber << ", chan=" << fedchannel
              << std::endl;
    assert(0);
  }
...
void PixelNameTranslation::writeASCII(std::string dir) const {
  ...
  out << "# name                          TBMchannel  FEC      mfec  mfecchannel hubaddress portadd rocid     FED     "
         "channel     roc#"
      << endl;
  ...
  for (; iroc != translationtable_.end(); ++iroc) {
    ...
    out << iroc->first.rocname() << "       " << TBMChannel << "       " << iroc->second.fecnumber() << "       "
        << iroc->second.mfec() << "       " << iroc->second.mfecchannel() << "       " << iroc->second.hubaddress()
        << "       " << iroc->second.portaddress() << "       " << iroc->second.rocid() << "         "
        << iroc->second.fednumber() << "       " << iroc->second.fedchannel() << "       "
        << iroc->second.fedrocnumber() << endl;
  }
...
bool PixelNameTranslation::ROCexists(PixelROCName theROC) {
  if (translationtable_.find(theROC) == translationtable_.end()) {
    return false;
  }
  return true;
}
