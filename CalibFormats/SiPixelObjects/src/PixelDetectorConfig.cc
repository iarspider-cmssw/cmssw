...
PixelDetectorConfig::PixelDetectorConfig(std::string filename) : PixelConfigBase("", "", "") {
  std::string mthn = "[PixelDetectorConfig::PixelDetectorConfig()]\t\t    ";
  ...
  if (filename[filename.size() - 1] == 't') {
    ...
    if (in.eof())
      std::cout << mthn << "EOF after reading first module name" << std::endl;
...
unsigned int PixelDetectorConfig::getNModules() const { return modules_.size(); }

PixelModuleName PixelDetectorConfig::getModule(unsigned int i) const { return modules_[i]; }
...
std::map<unsigned int, std::set<unsigned int> > PixelDetectorConfig::getFEDsAndChannels(
    PixelNameTranslation *translation) const {
  //      FED Number                channels
...
void PixelDetectorConfig::writeASCII(std::string dir) const {
  std::stringstream s;
  s << __LINE__ << "]\t[PixelDetectorConfig::writeASCII()]\t\t    ";
...
void PixelDetectorConfig::writeXMLTrailer(std::ofstream *outstream,
                                          ...
                                          std::ofstream *out2stream) const {
  std::stringstream s;
  s << __LINE__ << "]\t[PixelDetectorConfig::writeXMLTrailer()]\t\t\t    ";
  std::string mthn = s.str();
