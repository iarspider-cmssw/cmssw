...
HcalCoderDb::HcalCoderDb(const HcalQIECoder& fCoder, const HcalQIEShape& fShape) : mCoder(&fCoder), mShape(&fShape) {}
...
template <>
void HcalCoderDb::adc2fC_<QIE11DataFrame>(const QIE11DataFrame& df, CaloSamples& clf) const {
  clf = CaloSamples(df.id(), df.samples());
  for (int i = 0; i < df.samples(); i++) {
    clf[i] = mCoder->charge(*mShape, df[i].adc(), df[i].capid());
    if (df[i].soi())
...
template <class Digi>
void HcalCoderDb::fC2adc_(const CaloSamples& clf, Digi& df, int fCapIdOffset) const {
  df = Digi(clf.id());
  df.setSize(clf.size());
...
template <>
void HcalCoderDb::fC2adc_<QIE10DataFrame>(const CaloSamples& clf, QIE10DataFrame& df, int fCapIdOffset) const {
...
template <>
void HcalCoderDb::fC2adc_<QIE11DataFrame>(const CaloSamples& clf, QIE11DataFrame& df, int fCapIdOffset) const {
  int presample = clf.presamples();
...
void HcalCoderDb::fC2adc(const CaloSamples& clf, HBHEDataFrame& df, int fCapIdOffset) const {
  fC2adc_(clf, df, fCapIdOffset);
}
void HcalCoderDb::fC2adc(const CaloSamples& clf, HFDataFrame& df, int fCapIdOffset) const {
  fC2adc_(clf, df, fCapIdOffset);
}
void HcalCoderDb::fC2adc(const CaloSamples& clf, HODataFrame& df, int fCapIdOffset) const {
  fC2adc_(clf, df, fCapIdOffset);
}
void HcalCoderDb::fC2adc(const CaloSamples& clf, ZDCDataFrame& df, int fCapIdOffset) const {
  fC2adc_(clf, df, fCapIdOffset);
}
void HcalCoderDb::fC2adc(const CaloSamples& clf, HcalCalibDataFrame& df, int fCapIdOffset) const {
  fC2adc_(clf, df, fCapIdOffset);
}
void HcalCoderDb::fC2adc(const CaloSamples& clf, QIE10DataFrame& df, int fCapIdOffset) const {
  fC2adc_(clf, df, fCapIdOffset);
}
